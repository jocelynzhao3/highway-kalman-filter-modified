...
public:
	...
	Tools tools;
	bool pass = true;
	// RMSE thresholds used for simple pass/fail checks in the demo.
	// These are user-facing acceptance limits; tuning them does not
	// change estimator behavior directly but alters when the demo flags
	// the filter as failed. The real RMSE improvement comes from tuning
	// process/measurement noise and covariances in the UKF (see ukf.cpp).
	std::vector<double> rmseThreshold = {0.30,0.16,0.95,0.70};
	// Logged RMSE values when a threshold is exceeded (keeps a record for
	// visualization). Not used for tuning the filter itself.
